# delay the timer by 10s | header got seen to the output
...
class RMSDeviceTesterApp:
    def __init__(self, root):
        ...
        ttk.Entry(setup_frame, textvariable=self.ap_name_var, width=12).grid(row=0, column=1, padx=5)

        ttk.Label(setup_frame, text="Role:").grid(row=0, column=2, sticky="w", padx=5)
        self.role_var = tk.StringVar(value="Viewer")
        self.role_cb = ttk.Combobox(setup_frame, textvariable=self.role_var, values=list(USER_CREDENTIALS.keys()), state="readonly", width=14)
        self.role_cb.grid(row=0, column=3, padx=5)
        self.role_cb.bind("<<ComboboxSelected>>", self.on_role_change)

        ttk.Label(setup_frame, text="Password:").grid(row=0, column=4, sticky="w", padx=5)
        self.password_var = tk.StringVar(value=USER_CREDENTIALS["Viewer"]["password"])
        ttk.Entry(setup_frame, textvariable=self.password_var, width=14, show="*").grid(row=0, column=5, padx=5)

        ttk.Label(setup_frame, text="Session Token:").grid(row=0, column=6, sticky="w", padx=5)
        self.session_token_var = tk.StringVar(value="")
        ttk.Entry(setup_frame, textvariable=self.session_token_var, width=20).grid(row=0, column=7, padx=5)

        # Execution Buttons
        self.run_btn = ttk.Button(setup_frame, text="Authenticate & Run", command=self.start_process)
        self.run_btn.grid(row=0, column=8, padx=5)

        self.cancel_btn = ttk.Button(setup_frame, text="Cancel Execution", command=self.cancel_process, state="disabled")
        self.cancel_btn.grid(row=0, column=9, padx=5)
        ...
        # Bottom Bar (Export Buttons & Status)
    ...
    def run_execution_pipeline(self):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                session_token = "3fa9a12da510c17d"
        except Exception:
            session_token = "3fa9a12da510c17d"
        ...
        # Step 2: Iterate through allowed APIs with 10s delay
        ...
        for idx, api in enumerate(apis):
            ...
            # 10 Seconds Delay with immediate Cancellation Check
            if idx < len(apis) - 1:

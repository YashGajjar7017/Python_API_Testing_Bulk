...
# Full Endpoints Specification with Default Payloads
API_ENDPOINTS = [
    {"sr": 1, "name": "Authentication API (Write API Login)", "url": "http://192.168.4.1/api/login", "method": "POST", "roles": {"Viewer": ["Write"], "Operator": ["Write"], "System Admin": ["Write"], "Security Admin": ["Write"]}, "payload": {"username": "viewer", "password": "viewer_001"}},
...
class RMSDeviceTesterApp:
    def __init__(self, root):
        ...
        # 1. Header Frame: Setup & Login Controls
        setup_frame = ttk.LabelFrame(root, text=" Device Connection & Authentication Setup ", padding=8)
        setup_frame.pack(fill="x", padx=10, pady=5)

        ttk.Label(setup_frame, text="AP Name:").grid(row=0, column=0, sticky="w", padx=3)
        self.ap_name_var = tk.StringVar(value="RMS-0001")
        ttk.Entry(setup_frame, textvariable=self.ap_name_var, width=10).grid(row=0, column=1, padx=3)
        ...
        ttk.Entry(setup_frame, textvariable=self.password_var, width=13, show="*").grid(row=0, column=5, padx=3)

        # Login Action Button
        ttk.Button(setup_frame, text="1. Fetch Session Token (/api/login)", command=self.fetch_session_token).grid(row=0, column=6, padx=8)

        ttk.Label(setup_frame, text="Session Token (Paste or Auto):").grid(row=0, column=7, sticky="w", padx=3)
        self.session_token_var = tk.StringVar(value="")
        ttk.Entry(setup_frame, textvariable=self.session_token_var, width=22).grid(row=0, column=8, padx=3)
        ...
        # 2. Preview Frame Upside: Real-time Request Details (Method, Header, Payload)
        ...
        # 3. Middle Paned Window (Left Table & Right Response Output)
        paned = ttk.PanedWindow(root, orient="horizontal")
        paned.pack(fill="both", expand=True, padx=10, pady=5)

        # Left API Table Frame
    ...
    def update_request_preview(self, method, url, headers, payload):
        ...
        preview_str += f"PAYLOAD: {json.dumps(payload, indent=2) if payload else 'None'}"
    ...
    def fetch_session_token(self):
        ...
        # Show preview for login request
        self.update_request_preview("POST", login_url, headers, payload)
        self.update_status("Sending /api/login POST request...")
        ...
        def _login_thread():
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    self.update_status(f"Login Success! Session Token Acquired: {token}")
                    self.display_output({"name": "Login API", "url": login_url, "status": f"{resp.status_code} OK", "body": resp.text})
                else:
                    self.update_status(f"Login Failed ({resp.status_code}). Manual token paste allowed.")
                    self.display_output({"name": "Login API", "url": login_url, "status": f"{resp.status_code} Error", "body": resp.text})
            except Exception as ex:
                self.update_status("Login Request Failed/Timeout. You can paste a session token manually.")
                self.display_output({"name": "Login API", "url": login_url, "status": "Connection Error", "body": str(ex)})
    ...
    def start_process(self):
        token = self.session_token_var.get().strip()
        if not token:
            messagebox.showwarning("Missing Token", "Please click 'Fetch Session Token' or paste a valid session token first.")
            return
    ...
    def run_execution_pipeline(self):
        ...
        for idx, api in enumerate(apis):
            ...
            # Update preview at the top
            payload = api.get("payload")
            if api["name"] == "Authentication API (Write API Login)":
                payload = {"username": USER_CREDENTIALS[role]["username"], "password": self.password_var.get().strip()}

            self.root.after(0, lambda a=api, p=payload: self.update_request_preview(a["method"], a["url"], headers, p))
            ...
            # 10 Seconds Delay with Cancellation Monitoring
            if idx < len(apis) - 1:

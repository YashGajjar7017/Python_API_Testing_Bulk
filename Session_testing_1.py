# make only directly to the device got connected, without connection to the device how the api got check
# 1. the procedure is to the device ap name RMS-xxxx & password should enter like 4 auth user 1. viewer, 2.operator, 3. system-admin 4. security-admin
# 2. password should respestivly as follow 1. viewer_001, 2. operator_001, 3. sysadmin_001, 4. secadmin_001, when the 4 of 1 user got picked then automatically paste that pasted & make the send button, in the reponce it will given the session token
# 3. enter the session token in header -> cookie , session={sessionToken}
# 4. then after checking the 1 by 1 api checking & make the 30 sec delay between 2 apis call
# 5. also make the xcel and pdf with device name onto pdf should be given & also make the sideways textarea what says the output of the given seend api

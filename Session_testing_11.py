...
# Complete Master API Mapping (Session 11 updated sequence/roles)
API_ENDPOINTS = [
...
class RMSDeviceTesterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("RMS Device Direct AP Suite (v11)")
        ...
        ttk.Label(btn_bar, text=" (Checkboxes '☑' indicate API execution completion status)").pack(side="left", padx=5)
        ...
        self.tree.heading("read_status", text="Read Status")
        self.tree.heading("write_status", text="Write Status")
        ...
        self.tree.column("read_status", width=110, anchor="center")
        self.tree.column("write_status", width=110, anchor="center")
    ...
    def update_timer_loop(self):
        ...
        if self.session_expire_time:
            ...
            if remaining > 0:
                ...
                # If pipeline is running, let it auto-refresh when it executes next API
    ...
    def auto_refresh_token_synchronous(self):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                self.session_expire_time = time.time() + 60
                self.root.after(0, lambda: self.timer_lbl.config(text="Token Timer: 60s remaining", foreground="blue"))
                return True
        except Exception:
            pass
        return False
    ...
    def populate_table(self):
        ...
        for api in API_ENDPOINTS:
            ...
            has_read_capability = ("GET" in method_upper or "GET/POST" in method_upper or api["sr"] in [3, 5, 7, 10, 13, 17, 24])
            has_write_capability = ("POST" in method_upper or "GET/POST" in method_upper)

            if has_read_capability:
                read_status = "Pending" if "Read" in permissions else "Unauthorized"
            else:
                read_status = "-"

            if has_write_capability:
                write_status = "Pending" if "Write" in permissions else "Unauthorized"
            else:
                write_status = "-"
            ...
            self.tree.insert("", "end", iid=str(api["sr"]), values=(
                tick, api["sr"], api["name"], api["method"], api["url"], read_access, write_access, read_status, write_status
            ))

    def on_tree_click(self, event):
        # Tickboxes are now progress indicators, manual click-toggling is disabled
        pass

    def verify_payload_match(self, payload, response_text):
        if not payload:
            return True, "No payload to compare"
        try:
            resp_data = json.loads(response_text)
        except Exception:
            # Substring comparison if response is not JSON
            if isinstance(payload, dict):
                for k, v in payload.items():
                    if isinstance(v, (list, dict)):
                        continue
                    if isinstance(v, (str, int, float)) and str(v).lower() not in response_text.lower():
                        return False, f"Mismatch: '{v}' not found"
                return True, "Match (substring text match)"
            else:
                if str(payload).lower() in response_text.lower():
                    return True, "Match (text match)"
                return False, "Mismatch: payload text not found"
        ...
        if isinstance(payload, dict) and isinstance(resp_data, dict):
            ...
            for k, v in payload.items():
                ...
                resp_val = None
                found_key = None
                for rk in resp_data.keys():
                    if rk.lower() == k.lower():
                        resp_val = resp_data[rk]
                        found_key = rk
                        break
                
                if found_key is None:
                    # If key itself is not in response, check if the value exists in response values
                    flat_vals = [str(val).lower() for val in resp_data.values()]
                    if str(v).lower() not in flat_vals:
                        mismatches.append(f"{k} missing")
                else:
                    if str(resp_val).lower() != str(v).lower():
                        mismatches.append(f"{k}: expected {v}, got {resp_val}")
    ...
    def start_process(self):
        ...
        # Execute all APIs sequentially
        selected_sr_list = [api["sr"] for api in API_ENDPOINTS]

        # Reset all checkboxes to empty "☐" in treeview before starting execution
    ...
    def run_execution_pipeline(self, selected_sr_list):
        ...
        # Write API to Read API validation mapping
        WRITE_TO_READ_MAPPING = {
            3: 4,   # UUID Checking Single-phase POST -> GET
            5: 6,   # ISP Configuration API POST -> GET
            7: 8,   # Remote Server POST -> GET
            10: 11, # Secure Broker POST -> GET
            13: 12, # Inverter POST -> GET
            17: 16, # WIFI Check POST -> GET
            24: 23, # MQTTServer POST -> GET
            26: 26  # Modbus POST -> GET (self-check)
        }
        ...
        for idx, api in enumerate(apis_to_run):
            ...
            # Auto-login and resume where it stopped if token is expired
            if self.session_expire_time and time.time() >= self.session_expire_time:
                self.update_status(f"Token expired at API Sr. #{api['sr']}. Auto-refreshing...")
                success = self.auto_refresh_token_synchronous()
                if not success:
                    self.update_status(f"Token refresh failed at API Sr. #{api['sr']}. Stopped.")
                    break
                else:
                    self.update_status(f"Resumed execution pipeline at API Sr. #{api['sr']}...")
                    time.sleep(0.5)
            ...
            # Force both read and write capability testing for all APIs
            has_read_capability = True
            has_write_capability = True

            # Mark running states in table
            r_status = "Running..."
            w_status = "Running..."
            self.update_status(f"Running API {idx+1}/{len(apis_to_run)}: {api['name']}...")
            self.tree.item(sr_id, values=("☑", api["sr"], api["name"], api["method"], api["url"], read_acc, write_acc, r_status, w_status))
            ...
            if isinstance(payload, dict):
                post_headers = {"Cookie": f"session={token}", "Content-Type": "application/json"}
            else:
                post_headers = {"Cookie": f"session={token}", "Content-Type": "text/plain"}

            get_headers = {"Cookie": f"session={token}"} if token else {}

            preview_headers = get_headers if api["method"].upper() == "GET" else post_headers
            self.root.after(0, lambda a=api, p=payload, h=preview_headers: self.update_request_preview(a["method"], a["url"], h, p))
            ...
            # Run Write check (POST)
            if has_write_capability:
                try:
                    if isinstance(payload, dict):
                        r = requests.post(api["url"], json=payload, headers=post_headers, timeout=5)
                    else:
                        r = requests.post(api["url"], data=payload, headers=post_headers, timeout=5)
                    write_status = f"{r.status_code} {r.reason}"
                    body_output += f"================ WRITE CHECK (POST) ================\nStatus: {write_status}\n\n{r.text}\n"
                except Exception as ex:
                    write_status = "Error/Offline"
                    body_output += f"================ WRITE CHECK ERROR ================\n{str(ex)}\n"

            # Run Read check (GET)
            if has_read_capability:
                try:
                    # Determine read URL: if this is a POST api with a separate GET url in map
                    sr_id_val = api["sr"]
                    if sr_id_val in WRITE_TO_READ_MAPPING:
                        read_sr = WRITE_TO_READ_MAPPING[sr_id_val]
                        if read_sr == sr_id_val:
                            read_api = {"url": api["url"], "name": api["name"] + " (Read check)"}
                        else:
                            read_api = next((a for a in API_ENDPOINTS if a["sr"] == read_sr), None)
                    else:
                        read_api = {"url": api["url"], "name": api["name"]}

                    if read_api:
                        time.sleep(0.3)
                        read_r = requests.get(read_api["url"], headers=get_headers, timeout=5)
                        
                        # Compare write payload values to read response values
                        if read_r.status_code == 200:
                            if payload:
                                is_match, match_msg = self.verify_payload_match(payload, read_r.text)
                                if is_match:
                                    read_status = "200 OK (Match)"
                                else:
                                    read_status = f"200 OK ({match_msg})"
                            else:
                                read_status = "200 OK"
                        else:
                            read_status = f"{read_r.status_code} {read_r.reason}"
            ...
            self.tree.item(sr_id, values=("☑", api["sr"], api["name"], api["method"], api["url"], read_acc, write_acc, read_status, write_status))
            ...
            # Delay before next request (reduced to 1s to allow completion within 1 min timer)
            if idx < len(apis_to_run) - 1:
                for remaining in range(1, 0, -1):
                    if self.cancel_event.is_set():
                        break
                    self.update_status(f"Waiting 1s delay... ({remaining}s remaining before next API)")
                    time.sleep(1)
    ...
    def display_output(self, res):
        ...
        output_data = (
            ...
            f"Read Status  : {res['read_status']} | Write Status: {res['write_status']}\n\n"
            f"================ RESPONSE BODY ================\n"
            f"{res['body']}"
        )
    ...
    def export_excel(self):
        ...
        ws.append([])
        ws.append(["Sr.", "API Name", "Method", "URL", "Read Access", "Write Access", "Read Status", "Write Status", "Response Output"])

        for item in self.execution_results:
            ws.append([item["sr"], item["name"], item["method"], item["url"], item["read_acc"], item["write_acc"], item["read_status"], item["write_status"], str(item["body"])[:500]])
    ...
    def export_pdf(self):
        ...
        table_data = [["Sr.", "API Name", "Method", "Read Acc", "Write Acc", "Read Status", "Write Status"]]
        for item in self.execution_results:
            table_data.append([str(item["sr"]), item["name"], item["method"], item["read_acc"], item["write_acc"], item["read_status"], item["write_status"]])

        t = Table(table_data, colWidths=[25, 180, 45, 50, 50, 75, 75])

# for the first api for getting the session_Token we need to send the body as given into photo 
...
# Role Default Passwords (Overridable via UI)
USER_CREDENTIALS = {
    "Viewer": "Admin@123",
...
class RMSDeviceTesterApp:
    def __init__(self, root):
        ...
        # 1. Header Frame: Connection Controls
        ...
        ttk.Entry(setup_frame, textvariable=self.password_var, width=14).grid(row=0, column=5, padx=3)
        ...
        # 2. Preview Frame Upside
        preview_frame = ttk.LabelFrame(root, text=" Request Preview (Outgoing API Header & Body) ", padding=5)
    ...
    def fetch_session_token(self):
        ...
        # Exact Login Payload structure requested
        payload = {
        ...
        def _login_thread():
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    self.update_status(f"Login Failed ({resp.status_code}). Manual token entry enabled.")
    ...
    def start_process(self):
        token = self.session_token_var.get().strip()
        if not token:
            messagebox.showwarning("Missing Token", "Please click 'Fetch Session Token' or enter a valid session token first.")
            return
